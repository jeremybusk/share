...
for i in {0..3}; do
    ...
    branch_name="dev"
	sudo docker exec ${container_name} bash -c "apt -y update; apt -y iputils-ping bridge-utils iproute2 install wget openssh-server; wget http://repo.pyr8.io:10002/rnode_${branch_name}_all.deb; apt -y install ./rnode_${branch_name}_all.deb; mkdir /var/lib/rnode; mkdir /root/.ssh; echo ${authorized_public_keys} >> /root/.ssh/authorized_keys; mkdir /var/run/sshd; service ssh start; apt --purge rnode; apt -y install rnode_0.2.1_all.deb; ${rnode_cmd}" 
done
